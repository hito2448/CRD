...
class BN_fuse_layer(nn.Module):
    ...
    def _forward_impl(self, x, x_assit):
        l1 = self.relu(self.bn2(self.conv2(self.relu(self.bn1(self.conv1(torch.cat([x[0], x_assit[0]], 1)))))))
        l2 = self.relu(self.bn3(self.conv3(torch.cat([x[1], x_assit[1]], 1))))
        l3 = self.relu(self.bn4(self.conv4(torch.cat([x[2], x_assit[2]], 1))))
